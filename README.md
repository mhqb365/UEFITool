...
This fork adds an experimental editing path for the `new_engine` branch, focused on practical binary replacement workflows:

- Restores GUI access to edit-related context menu actions where they are applicable.
- Adds raw replacement support that patches the opened image buffer directly by item offset.
- Allows replacement data to be larger than the original item data, then reparses the patched image so the tree and hex view refresh immediately.
- Saves the patched image buffer directly when no full reconstruction action is required.
- Adds initial, limited reconstruction helpers for firmware volumes, files, sections, and pad files.
- Adds a Windows `Build.bat` helper that builds with CMake/Ninja and prepares a packed release folder with `Run.bat`.

This is not a complete firmware reconstruction implementation yet. Header resizing, checksum recalculation, alignment/free space management, compressed or guided sections, FIT/BootGuard-sensitive updates, insert support, and most full rebuild paths still need more work and testing before this can be considered a complete firmware editor.
